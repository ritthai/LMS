...
data.scan(/;Code=(...?.?.?.?.?)&amp;/m) { |a|
	...
	print "<course>"
	data2.scan(/<center><table border=0 width=80%><tr><td align=left><B><a name = "(.*){4,8}"><\/a>.*{0,30}<\/b><\/td><td align=right>Course ID: .*{4,8}<\/td><\/tr><tr><td colspan=2><b>(.*)<\/B><\/td><\/tr><tr><td colspan=2>(.*)<\/td>/m) { |c|
		print "<code>"+c[0]+"</code>\n"
